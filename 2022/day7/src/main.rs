...
fn traverse_fs(fs: &Vec<Directory>, root: &Directory, padding: u32, running_total: &mut u32) -> u32 {
    ...
    for dir_idx in root.children.iter() {
        dir_total += traverse_fs(fs, &fs[*dir_idx as usize], padding + 4, running_total);
    }
    // println!("{dir_total}");
    if dir_total <= 100000 {
...
fn main() {
    ...
    traverse_fs(&fs, &fs[0], 0, &mut running_total);
    println!("ans = {running_total}");
}
